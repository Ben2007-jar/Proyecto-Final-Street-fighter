...
/**
 * Pantalla de juego en modo red.
 * Sincroniza el estado del juego entre servidor y cliente.
 */
public class NetworkGameScreen implements Screen {
    ...
    private static final float NETWORK_UPDATE_INTERVAL = 0.033f; // ~30 updates/segundo
    ...
    public NetworkGameScreen(Principal game, GameServer server, GameClient client, int playerId,
                            int selectedP1, int selectedP2, int selectedMap) {
        this.game = game;
        ...
        // Solo controlamos nuestro personaje
        if (playerId == 1) {
            controller1 = InputController.createPlayer1Controller();
            controller2 = new InputController(0, 0, 0, 0, 0, 0); // Dummy
        } else {
            controller1 = new InputController(0, 0, 0, 0, 0, 0); // Dummy
    ...
    private void update(float delta) {
        // Actualizar solo nuestro personaje
        if (playerId == 1) {
    ...
    private void sendGameState() {
        NetworkPacket packet = new NetworkPacket(NetworkPacket.PacketType.GAME_STATE);
        packet.playerId = playerId;
        
        // Enviar estado de nuestro personaje
        if (playerId == 1) {
            packet.posX = player1.getX();
            packet.posY = player1.getY();
            packet.health = player1.getHealth();
            packet.facingRight = player1.isFacingRight();
            packet.isAttacking = player1.isAttacking();
            packet.isKicking = player1.isKicking();
            packet.isBlocking = player1.isBlocking();
            packet.isJumping = player1.isJumping;
        } else {
            packet.posX = player2.getX();
            packet.posY = player2.getY();
            packet.health = player2.getHealth();
            packet.facingRight = player2.isFacingRight();
            packet.isAttacking = player2.isAttacking();
            packet.isKicking = player2.isKicking();
            packet.isBlocking = player2.isBlocking();
            packet.isJumping = player2.isJumping;
        }
    ...
    private void receiveGameState() {
        ...
        if (packet != null && packet.type == NetworkPacket.PacketType.GAME_STATE) {
            // Actualizar el personaje del oponente
            if (packet.playerId != playerId) {
                if (packet.playerId == 1) {
                    player1.setX(packet.posX);
                    player1.setY(packet.posY);
                    player1.health = packet.health;
                    player1.setFacingRight(packet.facingRight);
                    player1.isAttacking = packet.isAttacking;
                    player1.isKicking = packet.isKicking;
                    player1.isBlocking = packet.isBlocking;
                    player1.isJumping = packet.isJumping;
                } else {
                    player2.setX(packet.posX);
                    player2.setY(packet.posY);
                    player2.health = packet.health;
                    player2.setFacingRight(packet.facingRight);
                    player2.isAttacking = packet.isAttacking;
                    player2.isKicking = packet.isKicking;
                    player2.isBlocking = packet.isBlocking;
                    player2.isJumping = packet.isJumping;
                }
            }
        }
    }
